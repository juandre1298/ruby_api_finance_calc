...
def extract_data(
  ...
  data = TRM_data)
  return data.select { |row| DateTime.parse(row['Date']).to_date >= start && DateTime.parse(row['Date']).to_date <= finish }
end
...
def calculate_relative_month_price(data)
  ...
  data_per_month = months.map{ |m|
    ...
    first_value = current_month.last["Price"].to_f
    current_month.map{ |row| [row["Date"],first_value,row["Price"].to_f/first_value,(row["Price"].to_f/first_value-1)*1600*first_value] }
  }
end
